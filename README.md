...
**Note:** unlimited ammo is easy, but HP is tricky. if you see a potential offset that contains an exact value of your character HP (e.g. 320). That is not an actual HP value. It is just a HP bar (UI). That's why when you try to manipulate the value, it keeps changing to the original value. You can check it through disassembly or select "Find out what access this value". You would see there are addresses that keep accessing it. However, you can also edit assembly code by navigating to that pointer address then in disassembly editor, replace source (rax) with any integer value (e.g. 500) you wish the UI to show. But your character will die anyway when taking hits.
